...
class ProcessScanner:
    def __init__(self, log: logger.Log):
        ...
        self.executables: Dict[str, list] = {'posix': ['tf.sh', 'steam', 'Discord'],
    ...
    def parse_tasklist(self):
        ...
        if self.process_data['TF2']['running']:
            if not self.tf_win64_exe_is_tf2(self.parsed_tasklist[self.executables[os.name][0]]):
                self.log.debug(f"Found running non-TF2 tf_win64.exe with PID {self.parsed_tasklist[self.executables[os.name][0]]}")
                self.process_data['TF2'] = copy.deepcopy(self.p_data_default['TF2'])
    ...
    def tf_win64_exe_is_tf2(self, pid: int) -> bool:
        ...
        return configs.is_tf2_install(self.log, os.path.join(tf_win64_exe_dir, self.executables[os.name][0]))
